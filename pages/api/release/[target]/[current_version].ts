...
export default async function handler(
  ...
  try {
    ...
    for (const item of assets) {
      ...
      if (/msi\.zip\.sig$/.test(browser_download_url) && isWindowPlatform) {
      ...
      if (/AppImage\.tar\.gz$/.test(browser_download_url) && isMacOSPlatform) {
        platforms.macos = {
          url: browser_download_url,
          version: tag_name,
          notes: "",
      ...
      if (/app\.tar\.gz\.sig$/.test(browser_download_url) && isLinuxPlatform) {
        const { data: signature } = await axios(browser_download_url);
        platforms.linux = {
          ...platforms.linux,
          ...{
